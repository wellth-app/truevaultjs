...
function TrueVaultVaultStore(client, stringClient) {
	this.client = client;
  this.stringClient = stringClient;
}

TrueVaultVaultStore.prototype = {
	create: function(name,callback) {
		this.stringClient.post('/v1/vaults', jsonToFormString({ name : name }), handleResult(callback, false, true));
	},
	update: function(vaultId,name,callback)  {
		this.stringClient.put('/v1/vaults/' + vaultId, jsonToFormString({ name : name }), handleResult(callback, false, true));
	},
...
TrueVaultSchemaStore.prototype = {
	create: function(vaultId,document,callback) {
		this.client.post(
      '/v1/vaults/' + vaultId + '/schemas',
	...
	update: function(vaultId,documentId,document,callback)  {
		this.client.put(
      '/v1/vaults/' + vaultId + '/schemas/' + documentId, 
      { document : base64Encode(document) }, 
      handleResult(callback)
      );
	},
...
function TrueVault(apikey) {
	...
	this.vault = new TrueVaultVaultStore(this.client, this.stringClient);
